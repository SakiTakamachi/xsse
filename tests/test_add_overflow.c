...
static void test_mm_add_epi8_overflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi16_overflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi32_overflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi64_overflow(void **state)
{
	(void) state;
...
int main(void)
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(test_mm_add_epi8_overflow),
		cmocka_unit_test(test_mm_add_epi16_overflow),
		cmocka_unit_test(test_mm_add_epi32_overflow),
		cmocka_unit_test(test_mm_add_epi64_overflow)
	};
