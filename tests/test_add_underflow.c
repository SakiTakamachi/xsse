...
static void test_mm_add_epi8_underflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi16_underflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi32_underflow(void **state)
{
	(void) state;
...
static void test_mm_add_epi64_underflow(void **state)
{
	(void) state;
...
int main(void)
{
	const struct CMUnitTest tests[] = {
		cmocka_unit_test(test_mm_add_epi8_underflow),
		cmocka_unit_test(test_mm_add_epi16_underflow),
		cmocka_unit_test(test_mm_add_epi32_underflow),
		cmocka_unit_test(test_mm_add_epi64_underflow)
	};
